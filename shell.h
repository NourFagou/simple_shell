...
int handle_command(char *cmd, char *envp[]);
...
char **parse_pipeline_commands(char *pipeline, int *num_cmds);
...
void free_commands(char *cmds[], int num_cmds);

void execute_commands(char *cmds[], int num_cmds,
...
int my_strcmp(const char *stringA, const char *stringB);
...
char *my_strcat(const char *strA, const char *strB);
